...
async def get_win_probability(game_id: str) -> Optional[dict]:
    ...
    try:
        ...
        # Find indices for home, away, and optional event number
        home_pct_idx = None
        away_pct_idx = None
        event_num_idx = None
        for i, header in enumerate(headers):
            ...
            if "HOME" in header_upper and ("PCT" in header_upper or "PROB" in header_upper):
                ...
                if "PCT" in header_upper or "PROB" in header_upper:
                    away_pct_idx = i
            elif "EVENT_NUM" in header_upper or "EVENTNUM" in header_upper:
                event_num_idx = i
        ...
        # Build probability history (full time series for chart; frontend can slice if needed)
        probability_history = []
        for row in rows:
            try:
                ...
                if event_num_idx is not None and len(row) > event_num_idx:
                    try:
                        entry["event_num"] = int(row[event_num_idx])
                    except (ValueError, TypeError):
                        pass
                probability_history.append(entry)
        ...
        # NBA API often returns empty/non-JSON for games with no win-prob data (e.g. not started)
        err_str = str(e)
        if "Expecting value" in err_str or "JSONDecodeError" in err_str:
            logger.debug(f"No win probability data for game {game_id} (API returned empty or non-JSON)")
        else:
