...
def build_postgame_recap_prompt(game_data: dict) -> str:
    """
    Build prompt for post-game recap generation.
    game_data should include: final score, top performers, key moments, team stats.
    """
    ...
    key_moment_descriptions = game_data.get("key_moment_descriptions") or []
    away_top = game_data.get("away_top_performer", "N/A")
    home_top = game_data.get("home_top_performer", "N/A")

    return f"""Generate a concise post-game recap (3-4 sentences max) for this NBA game.
...
Key Moments: {", ".join(key_moment_descriptions[:3])}

Rules:
- 3-4 sentences only
- Name the winning team and final score in sentence 1
- Mention the top performer by name in sentence 2
- Reference one key moment in sentence 3
- No hype, no filler words
- Return only the recap text, no JSON, no markdown
"""
