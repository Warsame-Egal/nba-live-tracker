...
from app.constants import (
    ...
    CACHE_STALE_WARNING_AGE_SECONDS,
    GAME_STATUS_FINAL,
    GAME_STATUS_LIVE,
)
...
class LRUCache:
    ...
    def __init__(self, max_size: int) -> None:
        self._cache: OrderedDict[str, PlayByPlayResponse] = OrderedDict()
    ...
    def set(self, key: str, value: PlayByPlayResponse) -> None:
...
class DataCache:
    ...
    async def _poll_scoreboard(self) -> None:
        ...
        while True:
            try:
                try:
                    scoreboard_data = await asyncio.wait_for(getScoreboard(), timeout=50.0)
                    ...
                    async with self._lock:
                        age = time.time() - self._scoreboard_last_updated if self._scoreboard_last_updated else None
                    if age is not None and age > CACHE_STALE_WARNING_AGE_SECONDS:
                        logger.warning(
                            "Timeout fetching scoreboard; serving stale cache (last updated %.0fs ago). Will retry.",
                            age,
                        )
                    else:
                    ...
                    if age is not None and age > CACHE_STALE_WARNING_AGE_SECONDS:
                        logger.warning(
                            "Error fetching scoreboard (%s); serving stale cache (last updated %.0fs ago). Will retry.",
                            e,
                            age,
                        )
                    else:
                ...
                for game_id in games_to_poll:
                    ...
                    try:
                        playbyplay_data = await asyncio.wait_for(
                            getPlayByPlay(game_id), timeout=CACHE_PLAYBYPLAY_TIMEOUT_SECONDS
                        )
    ...
    def get_health_stats(self) -> Dict[str, Any]:
        """Return cache and polling stats for the health endpoint. Safe to call anytime."""
        try:
            return {
                "scoreboard_cached": self._scoreboard_cache is not None,
                "scoreboard_age_seconds": (
                    round(time.time() - self._scoreboard_last_updated, 1) if self._scoreboard_last_updated else None
                ),
                "playbyplay_games_cached": len(self._playbyplay_cache.keys()),
                "playbyplay_cache_max_size": self._playbyplay_cache._max_size,
                "active_games_tracked": len(self._active_game_ids),
                "scoreboard_task_running": self._scoreboard_task is not None and not self._scoreboard_task.done(),
                "playbyplay_task_running": self._playbyplay_task is not None and not self._playbyplay_task.done(),
            }
        except Exception:
            return {
