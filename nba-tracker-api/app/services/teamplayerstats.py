...
async def get_team_player_stats(team_id: int, season: str = "2024-25") -> TeamPlayerStatsResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Timeout fetching player stats. The NBA API may be slow or unavailable. Please try again later.",
        )
    except Exception as e:
        error_msg = str(e)
        ...
        if "Connection" in error_msg or "timeout" in error_msg.lower() or "resolve" in error_msg.lower():
            raise HTTPException(status_code=503, detail="Unable to connect to NBA API. Please try again later.")
