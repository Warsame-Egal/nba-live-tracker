...
async def getGamesForDate(date: str, db: AsyncSession) -> GamesResponse:
    ...
    try:
        ...
        async def fetch_from_db() -> GamesResponse | None:
            ...
            if rows:
                # On Render or if API fetch fails, rely solely on the database
                if RUNNING_ON_RENDER:
                    games_from_db = await fetch_from_db()
                    if games_from_db:
                        return games_from_db

            raise HTTPException(status_code=404, detail="Schedule data not available")

        # Try DB first even when not on Render
        games_from_db = await fetch_from_db()
        if games_from_db:
            return games_from_db
