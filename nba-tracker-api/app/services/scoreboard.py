    ...
    try:
        ...
        if not df_list or df_list[0].empty:
            raise HTTPException(status_code=404,
                                detail=f"No data found for team ID {team_id}")
        ...
        for game in df.to_dict(orient="records"):
            scheduled_game = ScheduledGame(
                ...
                field_goal_pct=float(game["FG_PCT"]) if game.get(
                    "FG_PCT") is not None else 0.0,
                three_point_pct=float(game["FG3_PCT"]) if game.get(
                    "FG3_PCT") is not None else 0.0,
                free_throw_pct=float(game["FT_PCT"]) if game.get(
                    "FT_PCT") is not None else 0.0,
                plus_minus=float(game["PLUS_MINUS"]) if game.get(
                    "PLUS_MINUS") is not None else 0.0
            )
            games.append(scheduled_game)
...
async def getCurrentTeamRecord(team_id: int) -> TeamDetails:
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Error fetching team standings: {e}")
...
def fetchTeamRoster(team_id: int, season: str) -> TeamRoster:
    ...
    try:
        ...
        raw_roster = commonteamroster.CommonTeamRoster(
            team_id=team_id, season=season).get_dict()
        ...
        for row in player_data:
            ...
            players.append(Player(
                ...
                weight=int(
                    player_dict["WEIGHT"]) if player_dict["WEIGHT"] else None,
                birth_date=player_dict["BIRTH_DATE"] or None,
                age=int(player_dict["AGE"]) if player_dict["AGE"] else None,
                experience="Rookie" if str(
                    player_dict["EXP"]).upper() == "R" else str(
                    player_dict["EXP"]),
                # Update 'R' to 'Rookie
...
async def getPlayerDetails(player_id: int) -> PlayerSummary:
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Error fetching player details: {e}")
...
async def getPlayByPlay(game_id: str) -> PlayByPlayResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Error retrieving play-by-play: {str(e)}")
