...
def format_games_for_insights(games: list) -> List[Dict]:
    ...
    for game in games:
        if isinstance(game, dict):
            ...
            if getattr(game, "gameStatus", None) != GAME_STATUS_LIVE:
                continue
            home = getattr(game, "homeTeam", None) or {}
            away = getattr(game, "awayTeam", None) or {}
...
async def fetch_nba_scoreboard():
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_error = e
            logger.warning("Timeout fetching scoreboard from NBA API (attempt %s/3, timeout 15s)", attempt)
        except Exception as e:
            last_error = e
            logger.warning("Error fetching scoreboard from NBA API (attempt %s/3): %s", attempt, e)
        if attempt < 3:
            ...
            home_team=TeamBoxScoreStats(
                ...
                players=[
                    PlayerBoxScoreStats(
                        ...
                        field_goals_made=player["statistics"].get("fieldGoalsMade"),
                        field_goals_attempted=player["statistics"].get("fieldGoalsAttempted"),
                        free_throws_made=player["statistics"].get("freeThrowsMade"),
                        free_throws_attempted=player["statistics"].get("freeThrowsAttempted"),
                        rebounds_offensive=player["statistics"].get("reboundsOffensive"),
                        rebounds_defensive=player["statistics"].get("reboundsDefensive"),
                        fouls_personal=player["statistics"].get("foulsPersonal"),
                    )
...
async def get_hustle_box_score(game_id: str) -> dict | None:
    """
    Hustle stats for a game (contested shots, deflections, charges drawn, screen assists, etc.)
    via BoxScoreHustleV2. Returns None if data not available (e.g. game not played yet).
    """
    try:
        await rate_limit()
        raw = await asyncio.wait_for(
            asyncio.to_thread(lambda: BoxScoreHustleV2(game_id=game_id, **get_api_kwargs()).get_dict()),
            timeout=10.0,
        )
        box = raw.get("boxScoreHustle") or raw.get("boxScorehustle")
        if not box:
            return None
        # Return serializable dict; keys may be camelCase from API
        return {
        ...
        logger.warning("Hustle box score unavailable for game %s: %s", game_id, e)
        return None


async def get_advanced_box_score(game_id: str) -> dict | None:
    """Advanced box score (TS%, usage, net rating, PIE). Returns None if unavailable."""
    try:
        await rate_limit()
        raw = await asyncio.wait_for(
            asyncio.to_thread(lambda: BoxScoreAdvancedV3(game_id=game_id, **get_api_kwargs()).get_dict()),
            timeout=10.0,
        )
    except Exception as e:
        logger.warning("Advanced box score unavailable for game %s: %s", game_id, e)
        return None
    ...
    for rs in sets:
        if isinstance(rs, dict) and rs.get("name"):
            h, rows = rs.get("headers") or [], rs.get("rowSet") or []
            out[rs["name"]] = [dict(zip(h, row)) for row in rows]
    return out if len(out) > 1 else None


async def get_game_matchups(game_id: str) -> dict | None:
    """
    Who guarded who: matchup minutes, FG% against defender, switches.
    Via BoxScoreMatchupsV3. Returns None if not available.
    """
    try:
        await rate_limit()
        raw = await asyncio.wait_for(
            asyncio.to_thread(lambda: BoxScoreMatchupsV3(game_id=game_id, **get_api_kwargs()).get_dict()),
            timeout=10.0,
        )
        box = raw.get("boxScoreMatchups") or raw.get("boxscorematchups")
        if not box:
            return None
        return {
            "game_id": str(box.get("gameId", game_id)),
            **{k: v for k, v in box.items() if k != "gameId"},
        }
    except Exception as e:
        logger.warning("Matchups unavailable for game %s: %s", game_id, e)
        return None
