...
_key_moments_cache: Dict[str, List[Dict[str, Any]]] = {}
...
def _set_moment_context(moment_id: str, context: str) -> None:
...
async def _cleanup_finished_games() -> None:
...
def detect_scoring_run(
    ...
    """
    Detect if a team has scored 8+ unanswered points.

    Unanswered points = team scored, opponent did NOT score in between.
    Non-scoring plays (turnovers, fouls, timeouts) do NOT break a run.
    Only opposing scoring plays break the run.
    """
    if len(recent_plays) < 2:
        return False

    team_unanswered = 0
    plays_to_check = recent_plays[:20]  # last ~2.5 minutes

    for play in plays_to_check:
        play_team = play.get("team_tricode", "")
        action_type = play.get("action_type", "").lower()
        is_scoring = "shot" in action_type or "free throw" in action_type

        if not is_scoring:
            continue  # Non-scoring plays do not break or contribute to runs

        if play_team == team_tricode:
            ...
            # Opponent scored — run is broken
            break

    return team_unanswered >= 8
...
async def detect_key_moments(game_id: str) -> List[Dict[str, Any]]:
    """
    ...
    # Only detect moments for live games
...
async def generate_batched_moment_contexts(moments_with_game_info: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    try:
        if not groq_is_ready():
            logger.debug("Groq not available, skipping batched context generation")
            return {}
...
async def process_live_games() -> None:
    """
    ...
    try:
        ...
        # Get all live games
        live_games = [game.gameId for game in scoreboard_data.scoreboard.games if game.gameStatus == GAME_STATUS_LIVE]
