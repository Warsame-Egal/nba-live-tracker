...
**Types of key moments (see [architecture.md](architecture.md) for full table):**

- **Game-tying shot** — Score tied after the play; scoring play.
- **Lead change** — Different team leading after the play.
- **Scoring run** — 8+ unanswered points (run detection over last ~20 plays).
- **Clutch play** — Last ~2 minutes of regulation/OT, score within a few points.
- **Big shot** — Significant shot that shifts momentum (criteria in code).

**How it works:** Detection runs incrementally on new plays only. All moments needing AI context in a time window are batched into one Groq call. Context is cached in an LRU of 1000 entries. Moments are sent via WebSocket.
...
**Caching:** Predictions: 100 date+season entries, 30 min TTL, LRU. Team stats: 1 h TTL by season (used for win prob calculation).
