...
### Get games for a date (schedule)

```bash
curl http://localhost:8000/api/v1/schedule/date/2025-01-15
```

### Get today's scoreboard (cached)

```bash
curl http://localhost:8000/api/v1/scoreboard/today
```
...
### Get game detail (aggregated)

```bash
curl http://localhost:8000/api/v1/game/0022400123/detail
```
...
For full cache table and rationale, see **[architecture.md](architecture.md)**.

### Data cache

- **Scoreboard:** Polled every 8 s, cached in memory. WebSocket clients read from cache only.
- **Play-by-play:** Polled every 5 s per active game, LRU cache max 20 games; finished games evicted; cleanup every 5 min; entries older than 24 h purged.

### AI / Groq caches

- **Batched insights:** 50 batches, 20 lead-change; LRU.
- **Key moments context:** LRU 1000 entries.
- **Moment list:** Per game, 5 min window; cleared when game finished.
- **Predictions:** 100 date+season entries, 30 min TTL, LRU.
- **Win probability:** 20 games, 1 h TTL, LRU.

### Other

- **League leaders:** 5 min TTL by stat category + season (NBA API).
