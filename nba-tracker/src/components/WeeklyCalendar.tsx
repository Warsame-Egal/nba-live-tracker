...
const WeeklyCalendar = ({ selectedDate, setSelectedDate }: WeeklyCalendarProps) => {
  ...
  /**
   * When the user selects a date (clicks a day), show the week that contains that date.
   * Only depend on selectedDate so we do not override arrow navigation: when the user
   * clicks the left/right arrow we only update currentWeekStart in the handler and
   * selectedDate is unchanged, so this effect does not run and the week stays put.
   */
  useEffect(() => {
            ...
            <Box
              ...
              sx={{
                ...
                backgroundColor: isSelected
                  ...
                  : 'transparent',
                color: isSelected ? 'primary.main' : isToday ? 'primary.main' : 'text.secondary',
                position: 'relative',
                '&:hover': {
                  backgroundColor: isSelected
