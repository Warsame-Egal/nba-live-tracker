...
export interface MomentumPointClickPayload {
  gameTime: number;
  period: number;
  clock: string;
}
...
interface MomentumChartProps {
  ...
  onPointClick?: (point: MomentumPointClickPayload) => void;
}
...
const calculateGameTime = (period: number, clock: string): number => {
  ...
  const previousPeriods = period <= 4 ? (period - 1) * 12 : 4 * 12 + (period - 5) * 5;
...
const MomentumChart: React.FC<MomentumChartProps> = ({
  ...
  const CustomTooltip = ({
    ...
    active?: boolean;
    payload?: Array<{ payload: MomentumDataPoint }>;
  }) => {
      ...
      return (
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            ...
            >
              {homeTeam}: <strong>{data.homeScore}</strong> | {awayTeam}:{' '}
              <strong>{data.awayScore}</strong>
            </Typography>
            {!isTied && (
              <Typography
                variant="caption"
  ...
  // Quarter boundary times (game minutes) for vertical reference lines
  const quarterMarkers = useMemo(() => {
    const ends = [12, 24, 36, 48];
    const markers: Array<{ gameTime: number; timeLabel: string }> = [];
    for (const end of ends) {
      const point = chartData.find(d => d.gameTime >= end);
      if (point) {
        markers.push({ gameTime: end, timeLabel: point.time });
      }
    }
    return markers;
  }, [chartData]);

  // Find lead change points for annotations (hooks must run before any return)
  ...
  const leadingSections = useMemo(() => {
    ...
    let currentTeam: 'home' | 'away' | 'tied' =
      chartData[0]?.differential > 0 ? 'home' : chartData[0]?.differential < 0 ? 'away' : 'tied';
    let sectionStart = 0;

    for (let i = 1; i < chartData.length; i++) {
      const newTeam: 'home' | 'away' | 'tied' =
        chartData[i].differential > 0 ? 'home' : chartData[i].differential < 0 ? 'away' : 'tied';
  ...
  return (
    ...
    >
            ...
            <Chip
              ...
              sx={{
                ...
                fontSize: {
                  ...
                  sm: typography.size.captionSmall.sm,
                },
      ...
      <Box sx={{ width: '100%', height: { xs: 180, sm: 240 } }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            {/* Quarter markers - vertical dashed lines */}
            {quarterMarkers.map((q, idx) => (
              <ReferenceLine
                key={`q-${idx}`}
                x={q.timeLabel}
                stroke={gridColor}
            ...
            <Area
              ...
              onClick={(data: unknown) => {
                if (onPointClick && data && typeof data === 'object' && 'gameTime' in data) {
                  const point = data as MomentumDataPoint;
                  onPointClick({
            ...
            <Scatter
              ...
              fill={
                theme.palette.mode === 'dark' ? 'rgba(255, 193, 7, 0.8)' : 'rgba(255, 152, 0, 0.8)'
              }
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
          <Box
            sx={{
              ...
              borderRadius: '50%',
              backgroundColor:
                theme.palette.mode === 'dark' ? 'rgba(255, 193, 7, 0.9)' : 'rgba(255, 152, 0, 0.9)',
