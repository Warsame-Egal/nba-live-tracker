...
/** Parse ISO 8601 clock (e.g. PT12M00.00S) to minutes. */
...
/** Total game time in minutes (continuous timeline). Same as MomentumChart. */
function calculateGameTime(period: number, clock: string): number {
...
const PlayByPlay = ({ gameId, isLiveGame = false, scrollToGameTime }: PlayByPlayProps) => {
  ...
  // Scroll to play nearest to scrollToGameTime (from momentum chart click)
  useEffect(() => {
    if (scrollToGameTime == null || actions.length === 0) return;
    if (scrollToGameTime === lastScrolledToRef.current) return;
    const withTime = actions.map(p => ({
      play: p,
      gameTime: calculateGameTime(p.period ?? 1, p.clock ?? ''),
    }));
    let nearest = withTime[0];
    let best = Math.abs(nearest.gameTime - scrollToGameTime);
    for (const item of withTime) {
      const d = Math.abs(item.gameTime - scrollToGameTime);
      if (d < best) {
        best = d;
        nearest = item;
      }
    }
    const actionNumber = nearest.play.action_number;
    const el = document.querySelector(`[data-play-action-number="${actionNumber}"]`);
    if (el) {
  ...
  const playsByPeriod = actions.reduce(
    (acc, play) => {
          ...
          <Box key={period}>
            ...
            <Box sx={{ position: 'relative' }}>
              {periodPlays.map(play => {
                const isScore =
                  play.action_type?.toLowerCase().includes('shot') ||
            ...
            {periodIdx < periods.length - 1 && <Divider sx={{ my: { xs: 3, sm: 4 } }} />}
          </Box>
        );
      })}
    </Box>
