            ...
            >
              ...
              {conferenceRank && conference && (
                <span>
                  {' '}
                  • {conferenceRank}
                  {getOrdinalSuffix(conferenceRank)} in {conference}
                </span>
              )}
            </Typography>
          )}
            ...
            >
              {teamStats.ppg && (
                <Box>
                  <Typography
                    variant="caption"
                    sx={{ display: 'block', mb: 0.5, opacity: 0.8, fontSize: '0.75rem' }}
                  >
                    PPG
                  </Typography>
                  <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.875rem' }}>
                    {teamStats.ppg.rank}
                    {getOrdinalSuffix(teamStats.ppg.rank)} • {teamStats.ppg.value.toFixed(1)}
                  </Typography>
                </Box>
              )}
              ...
              {teamStats.rpg && (
                <Box>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.875rem' }}>
                    {teamStats.rpg.rank}
                    {getOrdinalSuffix(teamStats.rpg.rank)} • {teamStats.rpg.value.toFixed(1)}
                  </Typography>
                </Box>
              )}
              ...
              {teamStats.apg && (
                <Box>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.875rem' }}>
                    {teamStats.apg.rank}
                    {getOrdinalSuffix(teamStats.apg.rank)} • {teamStats.apg.value.toFixed(1)}
                  </Typography>
                </Box>
              )}
              ...
              {teamStats.oppg && (
                <Box>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 600, fontSize: '0.875rem' }}>
                    {teamStats.oppg.rank}
                    {getOrdinalSuffix(teamStats.oppg.rank)} • {teamStats.oppg.value.toFixed(1)}
                  </Typography>
                </Box>
              )}
            </Box>
