...
import { CheckCircle, Cancel, History } from '@mui/icons-material';
 ...
 */
const HistoricalPredictions: React.FC<HistoricalPredictionsProps> = ({ loading, error, data }) => {
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
        ...
        <Chip label={`${data.total_games} games`} size="small" variant="outlined" />
      </Box>
          ...
          <TableBody>
            {data.predictions.map(pred => {
              ...
              const totalScoreError =
                (pred.accuracy.score_error_home || 0) + (pred.accuracy.score_error_away || 0);
                  ...
                  <TableCell>
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {pred.prediction.predicted_away_score} -{' '}
                        {pred.prediction.predicted_home_score}
                      </Typography>
                    </Box>
                  </TableCell>
                  <TableCell>
