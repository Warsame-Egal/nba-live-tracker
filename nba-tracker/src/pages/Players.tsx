...
import { SeasonLeadersResponse, SeasonLeadersCategory } from '../types/seasonleaders';
...
const Players = () => {
  ...
  useEffect(() => {
    ...
    const fetchSeasonLeaders = async () => {
      if (activeTab === 'leaders') setLoading(true);
      try {
        const data = await fetchJson<{ data: SeasonLeadersCategory[] }>(
          `${API_BASE_URL}/api/v1/players/season-leaders?season=${encodeURIComponent(season)}&page=1&limit=20`,
          {},
          { maxRetries: 3, retryDelay: 1000, timeout: 30000 },
        );
        ...
        if (activeTab === 'leaders') setLoading(false);
      }
    };
    fetchSeasonLeaders();
    return () => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only refetch when season changes
  }, [season]);
  ...
  const sidebarLeaderCategories = useMemo(() => {
    if (!seasonLeaders?.categories) return [];
    const find = (keys: string[]) =>
      seasonLeaders!.categories.find(cat => {
          ...
          <>
            ...
            <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
              <Skeleton
                variant="rectangular"
                height={60}
                sx={{ borderRadius: borderRadius.md, mb: 1 }}
              />
              {[...Array(10)].map((_, index) => (
                <Skeleton
                  key={index}
  ...
  const renderSeasonLeaders = () => (
    <Box sx={{ minHeight: { xs: 400, sm: 500 } }}>
      <Box
        sx={{ mb: 3, display: 'flex', justifyContent: 'flex-end', minHeight: { xs: 40, sm: 48 } }}
      >
        <FormControl size="small" sx={{ minWidth: { xs: 160, sm: 180 } }}>
  ...
  return (
    ...
    >
      <PageContainer maxWidth={1400} sx={{ px: responsiveSpacing.container, py: responsiveSpacing.containerVertical }}>
        ...
        {/* Tab Content + Sidebar (two-column on lg+) */}
        <Box
          sx={{
            display: 'grid',
        ...
        >
          ...
          >
            {sidebarLeaderCategories.length > 0 ? (
              sidebarLeaderCategories.map(cat => (
                <Paper
                  key={cat.category}
                  elevation={0}
                  sx={{
              ...
              >
                <Typography variant="body2" color="text.secondary">
                  Season leaders load with the selected season.
                </Typography>
              </Paper>
            )}
          </Box>
