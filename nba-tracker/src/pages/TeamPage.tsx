   ...
   <div className="max-w-6xl mx-auto px-4 py-10"> 
    <div className="flex flex-col md:flex-row items-start gap-8"> 
     ...
     <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-sm mt-4"> 
      <p><span className="text-gray-400 uppercase">Founded:</span> {team.year_founded ?? "—"}</p> 
      <p><span className="text-gray-400 uppercase">Arena:</span> {team.arena ?? "—"}</p> 
      <p><span className="text-gray-400 uppercase">Capacity:</span> {team.arena_capacity ?? "—"}</p> 
      <p><span className="text-gray-400 uppercase">Owner:</span> {team.owner ?? "—"}</p> 
      <p><span className="text-gray-400 uppercase">GM:</span> {team.general_manager ?? "—"}</p> 
      <p><span className="text-gray-400 uppercase">Coach:</span> {team.head_coach ?? "—"}</p> 
     </div> 
     </div> 
    </div> 
     ...
     <div className="overflow-x-auto"> 
      ...
      <tbody> 
       {roster.players.map((player) => ( 
        <tr key={player.player_id} className="border-b border-neutral-700 hover:bg-neutral-800"> 
