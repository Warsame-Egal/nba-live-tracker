  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-900 text-white rounded-lg shadow-lg">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-300">{player.team_name ?? "Free Agent"} ({player.position ?? "N/A"})</p>
      </div>

      {/* Player Stats */}
      <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
        <PlayerStat label="Height" value={player.height} unit="" />
        <PlayerStat label="Weight" value={player.weight} unit="lbs" />
        <PlayerStat label="Points Per Game" value={player.points_per_game} unit="" />
        <PlayerStat label="Rebounds Per Game" value={player.rebounds_per_game} unit="" />
        <PlayerStat label="Assists Per Game" value={player.assists_per_game} unit="" />
      </div>
    </div>
  );
};
